	...
	public function store(Request $request) {
		...
		if($step == 333){
			...
			try {
				...
				if(isset($return_data_arr['has_error']) && !empty($return_data_arr['has_error'])){
					...
					if($request->input('application_no') == 0){
						...
						$request->session()->put('application_no', $application_no);
						$request->session()->put('ifausraccess', $xpassword);
					...
					$savemsg = "Thank you for applying as IFA! Your application has been submitted, an email has been sent to your email address.".$loginfo;

					Session::flash('firstregmessage', $savemsg);
			...
			$validMessage = [
				...
				'validation.uploaded' => 'Please check your image size(Max 1MB) or type(Only jpeg, jpg,png).',
				'validation.upload_picture' => 'Please check your image size(Max 1MB) or type(Only jpeg, jpg,png).',
				'upload_picture' => 'Please check your image size(Max 1MB) or type(Only jpeg, jpg,png).',
				'present_address_flat_no.required' => 'Present Address flat_no input value is wrong.',
				'present_address_house_no.required' => 'Present address house_no input value is wrong.',
		...
		if($step == 333){
			...
			try {
				...
				if(isset($return_data_arr['has_error']) && !empty($return_data_arr['has_error'])){
					...
					if($request->input('application_no') != 0){
						$application_no = $request->input('application_no');
						$existing_ifa_details = IFARegistration::where('application_no', $application_no)->first();

						$request->request->add(['mobile_no', $existing_ifa_details->mobile_no]);
						$request->request->add(['email', $existing_ifa_details->email]);
						
						unset($insert_arr['mobile_no']);
					...
					if(!$request->input('mobile_no') && isset($existing_ifa_details) && !empty($existing_ifa_details)){
						$mobile_no = $existing_ifa_details->mobile_no;
					}else{
						$mobile_no = $request->input('mobile_no');
					}

					if(!$request->input('email') && isset($existing_ifa_details) && !empty($existing_ifa_details)){
						$email = $existing_ifa_details->email;
					}else{
						$email = $request->input('email');
					}
